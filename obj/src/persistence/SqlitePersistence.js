...
class SqlitePersistence {
    ...
    constructor(tableName, schemaName) {
         ...
         */
        this._dependencyResolver = new pip_services3_commons_nodex_6.DependencyResolver(SqlitePersistence._defaultConfig);
        /**
         * The logger.
         */
    ...
    open(correlationId) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (this._connection == null) {
                throw new pip_services3_commons_nodex_5.InvalidStateException(correlationId, 'NO_CONNECTION', 'SQLite connection is missing');
            }
            if (!this._connection.isOpen()) {
                throw new pip_services3_commons_nodex_4.ConnectionException(correlationId, "CONNECT_FAILED", "SQLite connection is not opened");
            }
            ...
            try {
                ...
                this._client == null;
                throw new pip_services3_commons_nodex_4.ConnectionException(correlationId, "CONNECT_FAILED", "Connection to sqlite failed").withCause(ex);
            }
        });
    }
    /**
            ...
            if (pagingEnabled) {
                ...
                let count = yield new Promise((resolve, reject) => {
                    this._client.get(query, (err, result) => {
                        ...
                        let count = result ? pip_services3_commons_nodex_7.LongConverter.toLong(result.count) : 0;
                        resolve(count);
                    });
                });
...
SqlitePersistence._defaultConfig = pip_services3_commons_nodex_1.ConfigParams.fromTuples("collection", null, "dependencies.connection", "*:connection:sqlite:*:1.0", 
// connections.*
// credential.*
